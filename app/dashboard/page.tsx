...
export default function PedidosDashboard() {
    ...
    if (isLoading) {
        return <div className="p-6 text-center text-gray-500">Cargando datos de pedidos...</div>;
    }
        ...
        <div className="p-4 space-y-8 bg-white min-h-screen text-black">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2">
                    <Card className="shadow-lg">
                        <CardBody className="p-4">
                            <h3 className="text-lg font-semibold text-black mb-2">Ganancias Mensuales (Pedidos Entregados)</h3>
                            <ResponsiveContainer width="100%" height={300}>
                                <BarChart data={graficoGanancias}>
                                    ...
                                    <Bar dataKey="ganancias" fill="#007D8A" radius={[4, 4, 0, 0]} />
                                </BarChart>
                            </ResponsiveContainer>
                        </CardBody>
                    </Card>
                ...
                <div>
                    <Card className="shadow-lg">
                        <CardBody className="p-4">
                            <h3 className="text-lg font-semibold text-black mb-2">Distribución de Pedidos</h3>
                            <ResponsiveContainer width="100%" height={300}>
                                <PieChart>
                                    <Pie data={graficoStatus} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={100} label>
                                        ...
                                        pedidosPendientes.map(pedido => (
                                            <tr key={pedido._id} className="bg-white border-b hover:bg-gray-50">
                                                <td className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">
                                                    {pedido.usuario?.nombre || 'Usuario'} {pedido.usuario?.apellido || 'Eliminado'}
                                                </td>
                                                <td className="px-6 py-4">{pedido.id_usuarioVendedor?.nombre || 'N/A'}</td>
                                                <td className="px-6 py-4">{new Date(pedido.createdAt).toLocaleDateString()}</td>
                                                <td className="px-6 py-4 text-right font-semibold">${pedido.montoTotal.toLocaleString()}</td>
                                            </tr>
                                        ))
                                    ) : (
                                        <tr>
