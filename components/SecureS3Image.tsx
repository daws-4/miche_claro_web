...
export const SecureS3Image: React.FC<SecureS3ImageProps> = ({ s3Url, alt, className }) => {
    ...
    if (isLoading) {
        ...
        return <div className={`${className} bg-gray-200 animate-pulse rounded-t-xl`}></div>;
    }

    if (!presignedUrl) {
        // Muestra una imagen de fallback si falla
        return <img src={'https://placehold.co/600x400/EEE/31343C?text=Error'} alt={alt} className={className} />;
    }
